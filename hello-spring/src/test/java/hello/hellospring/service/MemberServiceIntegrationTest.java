...
//    원래 TEST를 진행할 때 이미 데이터가 존재하는 경우 발생하는 문제점을 해결하기 위해서 AfterEach를 사용하여 TEST를 진행하고나서 DB를 초기화하는 메서드를
//    @AfterEach 애너테이션을 사용하고 DB를 초기화하는 메서드를 작성하여 초기화를 진행했었는데, 
//    SpringBoot에서는 이를 지원하는 기능이 있다.
...
//    }
    ...
    @Test
//    @Commit
    // Transactional 애너테이션을 사용하여 DB에 반영이 되지 않도록 설정했지만, @Commit 애너테이션을 작성하면 DB에 반영되는 것을 확인할 수 있다.
    void 회원가입() {
        //given -> 무언가가 주어졌는데
        Member member = new Member(); // member 클래스에 대한 객체를 생성하고
        member.setName("spring"); // 새로 들어온 멤버의 이름을 hello라고 setter 메서드를 통해 설정합니다.
